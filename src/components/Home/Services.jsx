            ...
            <div className='container mx-auto px-5 py-16'>
                ...
                <div className='mt-16'>
                    <div className='grid grid-cols-4 gap-5'>
                        {services_data.map(({ content, image, title }, index) => <div key={index} className='cursor-pointer group bg-primary bg-opacity-10 shadow-md'>
                            <div className='aspect-[16/11] overflow-hidden'>
                                <Image draggable={false} alt={title} src={image} width={500} height={400} className='w-full h-full object-cover group-hover:scale-110 duration-200' />
                            </div>
                            <div className='px-4 py-6'>
                                <h4 className='text-primary text-xl font-bold'>{title}</h4>
                                <p className='text-sm mt-2 font-normal opacity-70'>{content}</p>
                            </div>
                        </div>)}
                    </div>
                </div>
