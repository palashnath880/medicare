...
export const convertTimeStringToDateObject = (timeString) => {
    ...
    if (!timeString && !(timeString.endsWith('AM') || timeString.endsWith('PM'))) {
        return null;
    }
...
export const isTimeBetween = (startTime, endTime, time) => {
    ...
    if (startTime === time) { // if start time equal to time
        return true;
    }

    if (endTime === time) { // if end time equal to time
        return true;
    }
...
export const uploadImage = (image) => new Promise(async (resolve, reject) => {
    try {
